# Диагностика процессов в Linux

## Описание задания

Написать bash-скрипт для диагностики процессов в Linux, который:
1. Находит процессы текущего пользователя
2. Определяет процесс с максимальным использованием CPU
3. Выводит карту памяти для указанного процесса
4. Собирает дополнительную информацию об открытых файлах
5. Сохраняет все результаты в отчетный файл `report.txt`

## Требования

- **Использование утилит**: `ps`, `top`, `lsof`, работа с `/proc`, `kill`
- **Сохранение результатов**: все команды и их вывод сохраняются в файл
- **Запуск**: `chmod +x diagnose.sh && ./diagnose.sh`

## Используемые утилиты

1. **`ps`** - информация о процессах
2. **`top`** - динамическое отображение процессов
3. **`lsof`** - список открытых файлов
4. **`/proc`** - виртуальная файловая система с информацией о процессах
...
Создайте файл `diagnose.sh` со следующим содержимым:
